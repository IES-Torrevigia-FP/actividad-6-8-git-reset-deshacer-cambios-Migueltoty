...
def azure_openai_chat(endpoint, api_key, deployment, messages):
    ...
    mode = 'v1' if '/openai/v1' in path else 'classic'
    ...
    if mode == 'v1':
        ...
        errors.append((response.status_code, url, (response.text or '')[:500]))
    else:
        for api_version in ['2025-01-01-preview', '2024-08-01-preview']:
...
def build_prompt(resumen: dict, readme_excerpt: str, reflex_excerpt: str):
    sys_prompt = (
        ...
        'Debes ser coherente con las metricas del JSON y no inventar faltantes.'
    )
    user_prompt = f"""
...
Extracto Reflexion 6.8 (max 8k chars, si existe):
```
{reflex_excerpt}
```
...
- Redacta retro.md con:
  ...
  4) Tabla | Criterio | Puntuacion maxima | Puntuacion obtenida | Comentario |.
  5) Recomendaciones (bullets).
- Usa la nota total total/10 del JSON sin alterarla.
- Esta actividad (6.8) se califica con rubrica: Git reset (max 4), Evidencias (max 3), Reflexion (max 3).
- No incluyas README como criterio de calificacion ni como area de mejora principal.
- Si en JSON aparece reset_checks.cp2_soft_reset.ok en true, reconoce explicitamente el uso de `git reset --soft`.
- Si en JSON aparece reset_checks.cp3_mixed_reset.ok en true, reconoce explicitamente el uso de `git reset` (mixed).
- Si en JSON aparece reset_checks.cp4_hard_reset.ok en true, reconoce explicitamente el uso de `git reset --hard` y su riesgo.
- Si en JSON aparece reset_checks.cp5_reset_head.ok en true, reconoce explicitamente la utilidad de `git reset HEAD` para deshacer staging.
- Espanol formal.
...
def main():
    ...
    reflex_excerpt = read_opt(Path('reflexion-6-8.md'))
